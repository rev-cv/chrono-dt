	...
	def _isDate(self, y, m, d, isGenerateError=False):
		...
		if 12 < m < 1:
			if isGenerateError is False: 
				...
				raise "<chrono._isDate()>: The month is indicated by numbers from 1 to 12"
		...
		if 0 <= H < 24:
			if 0 <= M < 60:
				if 0 <= S < 60:
					...
					raise "<chrono._isTime()>: The seconds are incorrect."
			elif isGenerateError is True:
				raise "<chrono._isTime()>: The minutes are incorrect."
		elif isGenerateError is True:
			raise "<chrono._isTime()>: The hours are incorrect."
	...
	def _isCountDaysInMonth(self, y, m, d, isGenerateError=False):
		...
		if d <= 0:
			if isGenerateError is False: 
				...
				raise "<chrono._isCountDaysInMonth()>: Day cannot be number 0"
		...
		if m in [1, 3, 5, 7, 8, 10, 12] and d <=31: 
			...
			if isGenerateError is False: 
				...
				raise "<chrono._isCountDaysInMonth()>: The specified day does not exist in the specified month."
