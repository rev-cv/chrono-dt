...
class Formatter(object):
	...
	def format(self, temp = r"%a %b %d %H0:%M0:%S0 %Y %Z"):
		...
		if regex in temp:
			temp = sub(regex, monthsRus[self.m], temp)
		...
		if regex in temp:
			temp = sub(regex, monthsRu[self.m], temp)
		...
		if regex in temp:
			temp = sub(regex, format_weekdays[self.getWeekday()][2], temp)

		regex = r'%wAR'	#сокращеная надпись дня недели до 2 символов на русском языке
		if regex in temp:
			temp = sub(regex, format_weekdays[self.getWeekday()][1], temp)

		regex = r'%wAE'	#сокращеная надпись дня недели до 2 символов на русском языке
		if regex in temp:
			temp = sub(regex, format_weekdays[self.getWeekday()][4], temp)

		regex = r'%wR'	#сокращеная надпись дня недели на русском языке
		if regex in temp:
			temp = sub(regex, format_weekdays[self.getWeekday()][0], temp)

		regex = r'%wE'	#сокращеная надпись дня недели на английском языке
		if regex in temp:
			temp = sub(regex, format_weekdays[self.getWeekday()][3], temp)
		...
		if regex in temp:
			temp = sub(regex, f'{format_weekdays[self.getWeekday()][5]}', temp)
